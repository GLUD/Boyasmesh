...
$auth = new PHP_API_AUTH(array(
	'authenticator'=>function($user,$pass){ 
		...
		// recibimos user pass verificamos que exista y que concuerde el usuario con la contraseña 
		// si es valida pasamos los datos a un objeto 
		

		function Auth($tabls,$params){
			$db = new dbMySQL();
			
			
				$q="SELECT * FROM user_auth WHERE email = '".$params['user']."' AND pass= '".$params['pass']."';";

				$response = $db->query($q,'fetch_object');
				...
				if(!$response){
					...
					if($response[0]->valid==0){
						...
						$data['data']['token']    = $_SESSION['csrf'];
						$data['data']['id']       = $response[0]->id;
						$data['data']['username'] = $response[0]->username;
						$data['data']['email']    = $response[0]->email;
						$data['data']['type']     = $response[0]->type;
				...
				// if($tabls[$i]=='admin') $response[0]->estado  =  'activo'; 

		

			

			if(!$data['success']) $data['message']='Usuario o Contraseña incorrectos !';
		...
		//$arr = array('alumnos','apoderado','tutor','admin');
		//$user = mysql_real_escape_string($user);
		//$pass = mysql_real_escape_string($pass);
		$params = array('user' => $user, 'pass' => $pass);
		
		Auth('user_auth',$params);
